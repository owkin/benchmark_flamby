...
class FLambyDataset(BaseDataset):
    ...
    def __init__(self, fed_dataset, model_arch, loss, num_clients, metric, seed=42, *args, **kwargs):
    ...
    def train_test_split_datasets(self):
        ...
        # Therefore one could have to reimplement it
    ...
    def get_data(self):
        ...
        set_seed(self.seed)
        self.trainval_datasets = [self.fed_dataset(center=i, train=True) for i in range(self.num_clients)]
        self.trainval_sizes = [len(d) for d in self.trainval_datasets]
        self.test_datasets = [self.fed_dataset(center=i, train=False) for i in range(self.num_clients)]
        ...
        return dict(train_datasets=self.train_datasets, val_datasets=self.val_datasets, test_datasets=self.test_datasets, model_arch=self.model_arch, metric=self.metric, loss=self.loss(), num_clients=self.num_clients, batch_size_test=self.batch_size_test)
